...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-14 sm:h-16 w-full max-w-full items-center justify-between border-b border-slate-200 bg-white/80 dark:bg-slate-900/80 dark:border-slate-800 px-3 sm:px-6 lg:px-8 backdrop-blur-md transition-colors duration-200">
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        {/* Mobile Hamburger Button */}
        <button
          type="button"
          onClick={openMobileSidebar}
          className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 lg:hidden transition"
          aria-label="Open mobile menu"
        >
          <Menu className="h-4 w-4 sm:h-5 sm:w-5" />
        </button>

        <span className="text-xs font-semibold px-1.5 sm:px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 dark:bg-blue-950/60 dark:text-blue-300 dark:border-blue-900 flex items-center gap-1.5 shrink-0">
          <ShieldCheck className="h-3.5 w-3.5" />
          <span className="hidden sm:inline">{t.nav.adminMode}</span>
          <span className="sm:hidden text-[10px] font-bold">ADMIN</span>
        </span>
      </div>
